...
export function Admin() {
  ...
  const handleDeleteClick=async(courierId)=>{
    try{
        await deleteAdminDetails(courierId); 
        populateCourierState();
    } catch(error){
        console.log(error);
    }
...
async function deleteAdminDetails(courierId) {
  try {
    const response = await axios.delete('http://localhost:8091/deleteCourierDetails/${courierId}');
  ...
  async function fetchCourierDetails() {
    try {
      const response = await axios.get("http://localhost:8091/fetchAllCourierDetails");
      console.log(response.data);
